...
<div class="body">
    <header id="header" data-plugin-options="{'stickyEnabled': true, 'stickyEnableOnBoxed': true, 'stickyEnableOnMobile': false, 'stickyStartAt': 147, 'stickySetTop': '-147px', 'stickyChangeLogo': false}">
        <div class="header-body">
            @include('customer.porto.layouts.header-top')
            @include('customer.porto.layouts.header')
            @include('customer.porto.layouts.navbar')
        </div>
    </header>
    @include('customer.porto.layouts.mobile-nav')
    <div role="main" class="main">
        @yield('content')
    </div>
    @include('customer.porto.layouts.footer')
</div>
